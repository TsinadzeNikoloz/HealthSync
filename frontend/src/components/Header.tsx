					...
					<div className="max-h-80 overflow-y-auto divide-y divide-slate-50">
						{notifications.length === 0 ? (
							...
							notifications.map((n) => (
								...
								>
									<div
										className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${typeBg[n.type] ?? typeBg.system}`}
									>
										<i
					...
					<>
						...
						<div className="flex items-center gap-4 group cursor-pointer">
							<div className="text-right hidden sm:block">
								...
								<p
									className={`text-[10px] font-extrabold uppercase tracking-widest ${user.role === 'ADMIN' ? 'text-rose-500' : user.role === 'DOCTOR' ? 'text-emerald-500' : 'text-indigo-500'}`}
								>
									{user.role}
								</p>
							</div>
