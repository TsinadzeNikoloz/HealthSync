									...
									>
										<td className="px-8 py-6">
											<div className="flex items-center gap-4">
												<div className="relative">
													...
													/>
													<div
														className={`absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border-2 border-white ${user.role === Role.ADMIN ? 'bg-rose-500' : user.role === Role.DOCTOR ? 'bg-indigo-500' : 'bg-emerald-500'}`}
													></div>
												</div>
												<div>
										...
										<td className="px-8 py-6 text-right">
											{user.id !== currentUser?.id ? (
												...
												<span className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic pr-2">
													Self Account
												</span>
											)}
										</td>
