	...
	return (
		<div className="space-y-10 animate-in fade-in duration-500">
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
				<StatCard
					label="Patients"
					value={users.filter((u) => u.role === Role.PATIENT).length}
					icon="fa-users"
					color="bg-blue-600"
				/>
				<StatCard
					label="Doctors"
					value={users.filter((u) => u.role === Role.DOCTOR).length}
					icon="fa-user-md"
					color="bg-indigo-600"
				/>
				<StatCard
			...
			<div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
				<div className="flex border-b border-slate-50">
					<button
						...
						className={`px-10 py-6 text-sm font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'users' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/30' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
					>
						Users
					</button>
					<button
						onClick={() => setActiveTab('services')}
						className={`px-10 py-6 text-sm font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === 'services' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/30' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
					>
						Services
					</button>
				</div>
									...
									<tbody className="divide-y divide-slate-50">
										{users.map((user) => (
											<tr key={user.id} className="group hover:bg-slate-50/50">
												...
												<td className="px-6 py-4">
													<span
														className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${user.role === Role.ADMIN ? 'bg-rose-50 text-rose-600' : user.role === Role.DOCTOR ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}
													>
														{user.role}
													</span>
												</td>
