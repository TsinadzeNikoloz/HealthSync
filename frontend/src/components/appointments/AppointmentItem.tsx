...
const AppointmentItem: React.FC<AppointmentItemProps> = ({
  ...
  const displayDate = appointment.date ? new Date(appointment.date).toLocaleDateString() : '';
  const displayTime = appointment.time || (appointment.date ? new Date(appointment.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '');
  ...
  return (
    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col md:flex-row items-center gap-6 card-hover group transition-all">
      ...
      <div className="flex flex-col items-center md:items-end gap-1 px-6 md:border-l border-slate-50">
        ...
        <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">{displayTime}</p>
      </div>
      ...
      {showConfirm && onDelete && (
        <ConfirmModal
          ...
          confirmLabel="Delete"
          onConfirm={() => { setShowConfirm(false); onDelete(appointment.id); }}
